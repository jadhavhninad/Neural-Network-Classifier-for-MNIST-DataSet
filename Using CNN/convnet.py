...
def initialize_multilayer_weights(m,net_dims,filtersz=3,convLcount=1,channel=1):
    '''
...
def multi_layer_forward(X, parameters,ch, convLcount=1):
...
def multi_layer_backward(dAL, caches, parameters, ch, convLcount=1):
    '''
    ...
    dA = dA.T
    nd = int(np.power((dA.shape[1]/ch), 0.5))
    #print(dA.shape)
    m=dA.shape[0]
    dA = dA.reshape(m,nd,nd,ch)
    #print(dA.shape)
    #print(dA.shape)
...
def classify(X, parameters):
    ...
    ch=5
    AL, caches = multi_layer_forward(X, parameters,ch)
    Amx = np.amax(AL,axis=0,keepdims=True)
    Nfactor = np.sum(np.exp(AL-Amx),axis=0,keepdims=True)
    Afin = np.exp(AL-Amx) / Nfactor

    YPred = np.argmax(Afin, axis=0)
    return YPred
...
def multi_layer_network(X, y, net_dims,  num_iterations=10, learning_rate=0.1):
    ...
    for ii in range(200):
        ...
        gradients = multi_layer_backward(dZ, caches, parameters,ch)
...
def main():
    ...
    trAcc = np.sum(train_Pred == train_label)/train_Pred.shape[0] *100
    teAcc = np.sum(test_Pred == test_label)/test_Pred.shape[0] *100
    ...
    plt.ylabel('error')
    plt.savefig('error_plot_multiLayer_conv2.png')
